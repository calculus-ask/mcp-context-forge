...
curl -X GET "https://mcpgateway-dev.example.net/admin/sso/providers" \
  -H "Authorization: Bearer ${MCPGATEWAY_BEARER_TOKEN}" \
  -H "Content-Type: application/json" | jq .
```

Expected response should include an ADFS provider:
...
```
https://mcpgateway-dev.example.net/auth/sso/login/adfs?redirect_uri=https://mcpgateway-dev.example.net/auth/sso/callback/adfs
```

Or use curl to get the authorization URL:

```bash
curl -X GET "https://mcpgateway-dev.example.net/auth/sso/login/adfs?redirect_uri=https://mcpgateway-dev.example.net/auth/sso/callback/adfs" \
  -L -v 2>&1 | grep -i location
```

This should redirect you to the ADFS authorization page.
...
**Solution**: The code checks multiple claims in order:
...
"Extracted user info from ADFS ID token for user: user@example.com"

# User creation/update
"Created new user via SSO: user@example.com (provider: adfs)"
```
